...
es = Elasticsearch(['ec2-52-40-68-150.us-west-2.compute.amazonaws.com'])
cluster = Cluster(['ec2-52-41-40-156.us-west-2.compute.amazonaws.com'])
...
def get_message(id):
    statement = 'SELECT * FROM test WHERE id={}'.format(id)
    response = session.execute(statement)
    json_response = [{'id': x.id, 'message': x.message} for x in response]
    return jsonify(messages=json_response)
...
def get_elastic_search_messages(keywords):
    ...
    return jsonify(transactions=result['hits']['hits'])
